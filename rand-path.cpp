...
void displayTrack(std::vector<std::vector<char>> maze, std::vector<int> xRoute, std::vector<int> yRoute){ //displays where player should go
    std::string end;
...
std::vector<std::vector<char>> mazeSolution(int xSize, int ySize){
    ...
    if(randomiseDelay == 0){
        ...
        delay = xSize / (3 - (rand() % 3)); //to delay potental turns so maze doesn't look so weird
    }
    ...
    //displayTrack(maze, xRoute, yRoute);
    return maze;
}
